  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Column(
            children: [
              ...
              Row(
                //아이콘을 맨 오른쪽으로 두기 위해서 row
                mainAxisAlignment: MainAxisAlignment.end, //아이콘 맨 오른쪽정렬 시키기
                children: [
                  //Icon(Icons.add,), //처음엔 이렇게 했는데 이러면 클릭이 안됨
                  IconButton(
                    //클릭하는 동작을 하고 싶어서 IconButton으로 만들기
                    onPressed: () {
                      //클릭됐을때
                      Navigator.push(
                        //Navigator로 다른 화면을 불러올 수 있음
                        context,
                        MaterialPageRoute(
                            builder: (context) =>
                                const AddToDoScreen() //AddToDoScreen의 route를 전달 -> AddToDoScreen로 화면 전환될거임
                            ),
        ...
        body: Padding(
          ...
          child: Column(
            //세로로 쭈욱 항목 적을거니까 Column 쓰기
            crossAxisAlignment:
                CrossAxisAlignment.start, //글씨 왼쪽 정렬 해주기위해 .start 사용
            children: [
              const SizedBox(
                //앱바랑 바디 사이에 조금의 공백 추가
                height: 10,
              ),
              Row(
                //글씨랑 아이콘을 나란히 둘거니까 Row 사용
                mainAxisAlignment:
                    MainAxisAlignment.spaceBetween, //글씨랑 아이콘 사이에 공백 넣기
                children: [
                  const Text(
                    "오늘의 할 일",
                    style: TextStyle(
